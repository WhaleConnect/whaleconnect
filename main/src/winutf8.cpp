...
std::wstring toWide(const char* from) {
    ...
    int stringSize = MultiByteToWideChar(CP_UTF8, 0, from, -1, nullptr, 0);
    ...
    MultiByteToWideChar(CP_UTF8, 0, from, -1, tmpBuf, stringSize);
...
std::string fromWide(const wchar_t* from) {
    ...
    int stringSize = WideCharToMultiByte(CP_UTF8, 0, from, -1, 0, 0, nullptr, nullptr);

    // Allocate char buffer to contain new string
    char* tmpBuf = new char[stringSize];

    // Convert the string to UTF-8 and store it in the buffer
    WideCharToMultiByte(CP_UTF8, 0, from, -1, tmpBuf, stringSize, nullptr, nullptr);
