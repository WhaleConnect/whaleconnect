...
int MAIN_FUNC(MAIN_ARGS) {
    ...
    int init = BTUtils::init(); // Initialize APIs for sockets and Bluetooth
    if (init != NO_ERROR) errorOutput.addError("Initialization failed - " + Sockets::formatLastErr());
...
static void drawBTConnOptions(const Sockets::DeviceData& target, uint16_t port, const std::string& extraInfo) {
    using Sockets::ConnectionType;
    using enum ConnectionType;
