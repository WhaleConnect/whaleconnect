...
const std::unordered_map<long, Sockets::NamedError> Sockets::errors = {
    ...
    E(WSATRY_AGAIN, "Temporary error in host name resolution."),
    E(WSANO_RECOVERY, "Non-recoverable error in host name resolution."),
#else
    E(EPERM, "Operation not permitted."),
    E(EBUSY, "The device or resource is currently in use."),
    E(EPIPE, "An operation was attempted on a socket that was shut down."),
    ...
    E(EAI_AGAIN, "Temporary error in host name resolution."),
    E(EAI_BADFLAGS, "Invalid flags value."),
    E(EAI_FAIL, "Non-recoverable error in host name resolution."),
    E(EAI_FAMILY, "The requested address family is not supported."),
    E(EAI_MEMORY, "No more memory available."),
    E(EAI_NODATA, "The host does not have any network addresses defined."),
    E(EAI_NONAME, "The name or service is unknown."),
